...
const notificationHeading = $("#notification-heading")[0];
...
convertBtnMorse.click(() => {
  if (inputF.value === "") showNotification("Please! Enter the Text Message");
  else {
...
copyBtnMorse.click(() => {
  if (outputF.value === "") showNotification("Nothing to Copy??");
  else {
    outputF.select();
...
//---------------------------------------------------------------------------------------------------
convertBtnTxt.click(() => {
  //-----check if the field is empty ot not-----
  if (outputF.value === "") showNotification("Please! Enter Morse Code");
  else {
...
//-------------------Txt Copy Button-------------------
copyBtnTxt.click(() => {
  if (inputF.value === "") showNotification("Nothing to Copy??");
  else {
    inputF.select();
...
function showNotification(heading) {
  //-----set heading of notification-----
  ...
  //-----auto remove notify panel
  timeOutNotify();
}

function setNotificationHeading(nHeading) {
  notificationHeading.innerText = nHeading;
}
...
//----------remove the notification panel after a specific time----------
